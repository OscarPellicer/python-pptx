...
class CT_TextParagraph(BaseOxmlElement):
    ...
    def append_text(self, text: str):
        ...
        if not text:
            print(f"DEBUG [CT_TextParagraph.append_text]: Cleared paragraph for empty text. Final XML: {self.xml}")
            return

        # ---Split text by \n or \v, creating a sequence of text parts and separators---
        # Using capturing group in re.split to keep delimiters: "a\vb" -> ['a', '\v', 'b']
        # An empty part can be generated by re.split if text starts/ends with delimiter
        # or has consecutive delimiters. e.g. "\va" -> ['', '\v', 'a']
        # These empty parts correctly represent empty runs if not delimiters.
        parts = re.split(r"(\n|\v)", text)
        print(f"DEBUG [CT_TextParagraph.append_text]: Text: {text!r}, Split parts: {parts!r}")

        for part in parts:
            if not part: # Skip empty strings that might result from split (e.g. if text was just "\n")
                continue

            if part == "\n" or part == "\v":
                print(f"DEBUG [CT_TextParagraph.append_text]: Adding <a:br/> for delimiter: {part!r}")
                # Create <a:br/> element directly
                br_xml = "<a:br %s/>" % nsdecls("a")
                br_elm = parse_xml(br_xml)
                self.append(br_elm)
            else:
                # This part is text for a run
                print(f"DEBUG [CT_TextParagraph.append_text]: Adding run for text: {part!r}")
                # Create <a:r><a:t>...</a:t></a:r> structure directly
                # Use existing static method from CT_RegularTextRun for escaping
                escaped_text = CT_RegularTextRun._escape_ctrl_chars(part)
                r_xml = "<a:r %s><a:t>%s</a:t></a:r>" % (nsdecls("a"), escaped_text)
                r_elm = parse_xml(r_xml)
                self.append(r_elm)
        
        print(f"DEBUG [CT_TextParagraph.append_text]: Final paragraph XML: {self.xml}")
